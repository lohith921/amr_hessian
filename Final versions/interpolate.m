function [u_new] = interpolate(o_mesh, o_sol, n_mesh)
...
N = zeros(n_ele,n_ele);                     % Nodal basis functions.
u_new = zeros(n_ele, n_ele);                % Solution vector for new mesh.
u_old = -1*ones(n_ele,1);                   % Solution vector for old mesh
W = zeros(n_ele, n_ele);                    % Weight matrix
...
for k = 1:n_ele
  ...
  N1Xg1 = 1 + ((m32 - 2*m21)*dx21*dx32)/denom;
