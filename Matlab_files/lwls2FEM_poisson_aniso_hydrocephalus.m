...
%baba=0;
%caca=1;
%newmesh.coords(boundary,1) =  2* mesh.coords(boundary,1)  ;
%newmesh.coords(boundary,2) =    mesh.coords(boundary,2) ;
%newmesh.coords(16,1)= 3;
%newmesh.coords(16,2)= 5;

% nonaffine transformation
%newmesh.coords(boundary,1) = mesh.coords(boundary,1).*mesh.coords(boundary,2)  ;
%newmesh.coords(boundary,2) = mesh.coords(boundary,2).^2;

% hydrocephalus
newmesh=readmesh2('hb_fin');
boundary = find(newmesh.border > 0);
